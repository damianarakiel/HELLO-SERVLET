...
/*Ten kod nadpisuje parametr*/
//        String passParam = httpRequest.getParameter("secret");
//        if(passParam != null) {
//            session.setAttribute("secret", passParam);
//        }

/*Ten kod pamięta parametr*/
        String passParam = httpRequest.getParameter("secret");
        session.setAttribute("secret" , passParam);

        if(SECRET.equals(session.getAttribute("secret")) || Integer.valueOf(1).equals(session.getAttribute("flags"))){
            session.setAttribute("flags", 1);
            filterChain.doFilter(servletRequest, servletResponse);
        } else {
            httpResponse.setStatus(HttpServletResponse.SC_FORBIDDEN);
        }
//        if("secret1234".equals(passParam)){
//            filterChain.doFilter(servletRequest, servletResponse);
//        } else {
//            httpResponse.setStatus(HttpServletResponse.SC_FORBIDDEN);
//        }
